...
public class RegisterActivity
{
    ...
    private void Validate(Trip? trip,RequestRegisterActivityJson request)
    {
        ...
        var validator = new ValidatorActivity();

        var result = validator.Validate(request);

        if((request.Date >= trip.StartDate && request.Date <= trip.EndDate) == false) 
        {
            result.Errors.Add(new ValidationFailure("Date", ResourceErrorMessages.ATIVIDADE_INDISPONIVEL));
        }
        ...
        if (result.IsValid == false)
        {
            ...
            throw new ErrorOrValidationException(errorMessage);
        }
    }
}
